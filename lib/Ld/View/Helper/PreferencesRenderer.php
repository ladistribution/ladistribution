    ...
    public function renderControl($preference)
    {
        ...
        switch ($preference['type']) {
            case 'boolean':
                ...
                echo '<input type="checkbox" value="1" id="' . $id . '"  name="' .
                    $inputName . '"' . $checked . ' />' . "\n";
                break;
            case 'password':
                echo '<input class="text" type="password" id="' . $id . '"  name="' .
                    $inputName . '" value="' . $this->view->escape($value) . '" />' . "\n";
                break;
            case 'range':
            ...
            case 'list':
                ...
                foreach ($preference['options'] as $option) {
                    ...
                    echo '  <option value="' . $option['value']  . '"' . $selected . '>' .
                        $option['label'] . '</option>' . "\n";
                }
                echo '</select>' . "\n";
                break;
            case 'hidden':
                echo '<input type="hidden" name="' . $inputName . '"
                    value="' . $this->view->escape($value) . '" />' . "\n";
                break;
            case 'textarea':
                echo '<textarea cols="46" rows="5" id="' . $id . '" name="' . $inputName . '">' .
                    $this->view->escape($value) . '</textarea>' . "\n";
                break;
            case 'text':
            default:
                echo '<input class="text" size="45" type="text" id="' . $id . '" name="' .
                    $inputName . '" value="' . $this->view->escape($value) . '" />' . "\n";
                break;
        }
    }
